...
create_namespace('app.Sandbox');


app.Sandbox.CreateViewController = function() {
    var that = this;
...
app.Sandbox.CreateViewController.prototype = {
    ...
    update_tracks_table: function(track){
        ...
        _.each(['artist', 'song', 'channel', 'created_at'], function(item){
            var newTrackCell = $(document.createElement('td'));
            if (item == 'created_at'){
                var created_at = _.first(track[item].split('T'));
                newTrackCell.text(created_at);
            }else{
                newTrackCell.text(track[item]);
            }

            newTrackRow.append(newTrackCell);
        });
